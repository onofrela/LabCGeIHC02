...
int main()
{
	...
	AlaIzquierda_M.LoadModel("Models/Ala_Izquierda.obj");
	AlaDerecha_M = Model();
	AlaDerecha_M.LoadModel("Models/Ala_Derecha.obj");
	...
	bool dragonflip = false;

	float tiempoAcumulado = 0.0f;
	float intervaloCambio = 50.0f;

	float progresoRejaIzquierdaUno = 0.0f;
	float progresoRejaIzquierdaDos = 0.0f;
	float progresoRejaDerecha = 0.0f;

	bool cambiaTextura = true;
	...
	while (!mainWindow.getShouldClose())
	{
		...
		model = glm::rotate(model, 180* dragonflip * toRadians, glm::vec3(0.0f, 0.0f, 1.0f));
		...
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.7f, 0.1f, -3.2f));
		model = glm::rotate(model, sin(angulovaria * toRadians) * 0.5f, glm::vec3(1.0f, 0.0f, 0.0f));
		/*color = glm::vec3(0.0f, 1.0f, 0.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));*/
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.7f, 0.9f, -2.8f));
		model = glm::rotate(model, -sin(angulovaria * toRadians) * 0.5f, glm::vec3(1.0f, 0.0f, 0.0f));
		/*color = glm::vec3(0.0f, 1.0f, 0.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));*/
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		*/
		...
		if (mainWindow.getRejaIzquierdaAbierta()) {
			if (progresoRejaIzquierdaUno < 1.0f) {
				progresoRejaIzquierdaUno += 0.05f * deltaTime;
			}
			else if (progresoRejaIzquierdaDos < 3.2f) {
				progresoRejaIzquierdaDos += 0.1 * deltaTime;
			}
		}
		else {
			if (progresoRejaIzquierdaDos > 0.0f) {
				progresoRejaIzquierdaDos -= 0.1 * deltaTime;
			} else if (progresoRejaIzquierdaUno > 0.0f) {
				progresoRejaIzquierdaUno -= 0.05f * deltaTime;
			}
		}
		model = glm::translate(model, glm::vec3(progresoRejaIzquierdaDos, 0.0f, progresoRejaIzquierdaUno));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		if (mainWindow.getRejaDerechaAbierta()) {
			if (progresoRejaDerecha > -90.0f) {
				progresoRejaDerecha -= 1.0f * deltaTime;
			}
		}
		else {
			if (progresoRejaDerecha < 0.0f) {
				progresoRejaDerecha += 1.0f * deltaTime;
			}
		}
		model = glm::rotate(model, glm::radians(progresoRejaDerecha), glm::vec3(0.0f, 1.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		blackMetalTexture.UseTexture();
		...
		//textura con movimiento
		//Importantes porque la variable uniform no podemos modificarla directamente
		toffsetletrerou += 0.003 * deltaTime;
